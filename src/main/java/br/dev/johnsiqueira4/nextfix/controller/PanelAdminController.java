...
public class PanelAdminController {
    ...
    public String actualizarRolUsuario(@PathVariable Long id, @RequestParam(required = false) String rol, @RequestParam(required = false) String nacionalidad, @RequestParam(required = false) LocalDate fechaNacimiento, @RequestParam(required = false) String email) {
        if (rol.equals("ROL_DIRECTOR") && nacionalidad != null && fechaNacimiento != null && email != null) {
            Director directorNuevo = new Director();
            directorNuevo.setNacionalidad(nacionalidad);
            directorNuevo.setFechaNascimiento(fechaNacimiento);
